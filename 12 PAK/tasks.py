...
valid_idx, train_idx  = train_test_split(range(len(train_set)), test_size=0.2, train_size=0.8)
...
valid_loader = torch.utils.data.DataLoader(
    train_set, batch_size=batch_size, num_workers=num_workers, sampler=valid_sampler)
...
for _ in (pbar := tqdm(range(epochs))):
    ...
    pbar.set_description(f'Acc: {valid_acc:.4f}  Avg. train/valid loss: {train_loss:.4f}/{valid_loss:.4f}')
